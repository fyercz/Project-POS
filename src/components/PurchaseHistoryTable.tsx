...
export const PurchaseHistoryTable: React.FC<PurchaseHistoryTableProps> = ({
  ...
  const [calibrateLiters, setCalibrateLiters] = useState<number>(tank.currentStockLiters);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
          ...
          {onDirectAdjustTank && (
            <button
              type="button"
              onClick={() => {
                setCalibrateLiters(Math.min(tank.totalCapacityLiters, tank.currentStockLiters));
              ...
              title="Koreksi atau kalibrasi manual sisa stok BBM tangki pendam"
            >
              <Wrench className="w-3.5 h-3.5 text-slate-500" />
              <span>Koreksi Stok Tangki</span>
            </button>
          )}
        </div>
      </div>
      ...
      {/* Manual Tank Calibration Modal */}
      {showCalibrateModal && onDirectAdjustTank && (
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="bg-slate-900 text-white p-4 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold">Koreksi & Kalibrasi Stok Tangki</h3>
                  <p className="text-xs text-slate-400">Atur ulang volume BBM fisik tangki pendam</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-5 space-y-4 text-xs">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-bold text-slate-900">{formatLiter(tank.totalCapacityLiters)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500 font-medium">Stok Sistem Saat Ini:</span>
                  <span className="font-mono font-bold text-blue-700">{formatLiter(tank.currentStockLiters)}</span>
                </div>
              </div>

              <div>
                <label className="block font-bold text-slate-700 mb-1">
                  Volume Stok Baru (Liter)
                </label>
                <div className="relative">
                  <input
                    ...
                    step="any"
                    value={calibrateLiters}
                    onChange={(e) => setCalibrateLiters(Math.min(tank.totalCapacityLiters, Math.max(0, parseFloat(e.target.value) || 0)))}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl font-mono text-sm font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-hidden"
                  />
                  <span className="absolute right-3 top-2.5 text-xs font-semibold text-slate-400">Liter</span>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  <button
                    type="button"
                    onClick={() => setCalibrateLiters(tank.totalCapacityLiters)}
                    className="px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg font-semibold border border-blue-200 transition-colors text-[11px]"
                  >
                    Pas Kapasitas ({formatLiter(tank.totalCapacityLiters)})
                  </button>
                  {tank.lastSoundingLiters !== undefined && tank.lastSoundingLiters > 0 && (
                    <button
                      type="button"
                      onClick={() => setCalibrateLiters(Math.min(tank.totalCapacityLiters, tank.lastSoundingLiters || 0))}
                      className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg font-semibold border border-emerald-200 transition-colors text-[11px]"
                    >
                      Tera Sounding Terakhir ({formatLiter(tank.lastSoundingLiters)})
                    </button>
                  )}
                  <button
                    type="button"
                    onClick={() => setCalibrateLiters(0)}
                    className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-semibold border border-slate-200 transition-colors text-[11px]"
                  >
                    Kosongkan (0 L)
                  </button>
                </div>
              ...
              <div className="pt-3 border-t border-slate-200 flex items-center justify-end gap-2">
                ...
                <button
                  type="button"
                  onClick={() => {
                    onDirectAdjustTank(calibrateLiters);
                    setShowCalibrateModal(false);
                  }}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-xs text-xs flex items-center gap-1.5"
                >
